...
function psw(){
    var psw = prompt('plz enter the password you set');
    if(psw.length >= 10){
        alert('Sorry, that is too long');
    }

    else if(psw.startsWith('*')){
        alert('Sorry, It should not start with *')
    }
    else if(psw.match('!') || psw.match('#') || psw.match('$') || psw.match('?')){
        alert('Sorry, It should not include ! $ # ?')
    }
    else{
        alert('That is good!');
    }
